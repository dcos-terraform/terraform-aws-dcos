...
cp cluster_profile.tfvars.example cluster_profile.tfvars
```
...
When complete, retrieve the key pair name and ensure that it matches the `ssh_key_name` in your [cluster_profile.tfvars](/aws/cluster_profile.tfvars.example). 

**Note**: The [cluster_profile.tfvars](/aws/cluster_profile.tfvars.example) always takes precedence over the [variables.tf](/aws/variables.tf) and is **best practice** for any variable changes that are specific to your cluster. 
...
```bash
terraform apply -var-file cluster_profile.tfvars -var state=upgrade -target null_resource.bootstrap -target null_resource.master -parallelism=1
