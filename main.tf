...
locals {
  ...
  dcos_exhibitor_address         = var.with_replaceable_masters ? module.dcos-infrastructure.lb_masters_internal_dns_name : ""
...
module "dcos-install" {
  ...
  bootstrap_os_user = coalesce(
    var.ansible_user,
    module.dcos-infrastructure.bootstrap_os_user,
  )
  ...
  dcos_aws_region                              = coalesce(var.dcos_aws_region, local.dcos_aws_region, false)
  ...
  dcos_exhibitor_address                       = coalesce(var.dcos_exhibitor_address, local.dcos_exhibitor_address, false)
  ...
  dcos_exhibitor_explicit_keys = coalesce(
    var.dcos_exhibitor_explicit_keys,
    local.dcos_exhibitor_explicit_keys,
    false
  )
  ...
  dcos_master_external_loadbalancer = coalesce(
    ...
    module.dcos-infrastructure.lb_masters_dns_name,
    false
  )
  ...
  dcos_num_masters                             = coalesce(var.dcos_num_masters, local.dcos_num_masters, false)
  ...
  dcos_s3_bucket = coalesce(
    var.dcos_s3_bucket,
    module.dcos-infrastructure.aws_s3_bucket_name,
    false
  )
  dcos_s3_prefix                  = coalesce(var.dcos_s3_prefix, local.dcos_s3_prefix, false)
